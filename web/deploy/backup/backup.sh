...
# audit log). Produces a single AES-256 encrypted tarball, rotates local copies,
# and optionally uploads off-box (DO Spaces / S3). Run by the systemd timer.
set -euo pipefail
...
file="$DEST/kyber-web-$stamp.tar.gz.enc"

# tar the data dir and encrypt the stream (nothing plaintext hits disk).
tar -C "$(dirname "$DATA_DIR")" -czf - "$(basename "$DATA_DIR")" \
  | openssl enc -aes-256-cbc -pbkdf2 -salt -pass "file:$PASSFILE" -out "$file"
chmod 600 "$file"
echo "backup: wrote $file ($(du -h "$file" | cut -f1))"

# Rotate: keep the newest $KEEP encrypted backups locally.
ls -1t "$DEST"/kyber-web-*.tar.gz.enc 2>/dev/null | tail -n +"$((KEEP + 1))" | xargs -r rm -f
